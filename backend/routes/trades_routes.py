...
async def get_trades_tree(
    ...
    """Public: returns [{id, name, trades:[{id, name, skills:[]}]}] for active categories/trades."""
    ...
    for cat in cats:
        ...
        # Ensure skills field is always present
        for t in trades:
            t.setdefault("skills", [])
...
# ─── Admin: Skills (3rd level) ────────────────────────────────────────────────

@router.get("/admin/trades/{trade_id}/skills")
async def list_skills(trade_id: str, admin: dict = Depends(require_admin)):
    """List all skills for a trade."""
    trade = await db.trades.find_one({"id": trade_id}, {"_id": 0})
    if not trade:
        raise HTTPException(status_code=404, detail="Trade not found")
    return {"skills": trade.get("skills", []), "trade_id": trade_id, "trade_name": trade["name"]}


@router.post("/admin/trades/{trade_id}/skills", status_code=201)
async def add_skill(trade_id: str, data: dict, admin: dict = Depends(require_admin)):
    """Add a skill to a trade (idempotent)."""
    ...
    if not trade:
        raise HTTPException(status_code=404, detail="Trade not found")
    if skill in trade.get("skills", []):
        raise HTTPException(status_code=400, detail="Skill already exists")
    await db.trades.update_one({"id": trade_id}, {"$addToSet": {"skills": skill}})
    return {"message": "Skill added", "skill": skill}


@router.delete("/admin/trades/{trade_id}/skills/{skill}")
async def remove_skill(trade_id: str, skill: str, admin: dict = Depends(require_admin)):
    """Remove a skill from a trade."""
    from urllib.parse import unquote
    skill = unquote(skill)
    result = await db.trades.update_one({"id": trade_id}, {"$pull": {"skills": skill}})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Trade not found")
