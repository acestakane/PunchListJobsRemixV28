...
async def get_public_profile(
    ...
    if is_crew_job_view:
        job_doc = await db.jobs.find_one(
            {"id": job_id},
            {"_id": 0, "crew_accepted": 1, "paid_reveals": 1}
        )
        if job_doc:
            is_accepted     = caller_id in job_doc.get("crew_accepted", [])
            has_paid_reveal = caller_id in job_doc.get("paid_reveals", [])
            should_mask     = not is_accepted and not has_paid_reveal
        else:
            should_mask = True  # job not found → hide contact by default
    elif viewing_crew_profile and not is_own_profile:
        # Hide crew contact unless they have an active unlock
        should_mask = not has_contact_unlock
    else:
        # For contractor profiles or own profile, don't mask
        should_mask = False

    if should_mask:
        profile.pop("phone", None)
        profile.pop("email", None)
        profile["contact_hidden"] = True
    else:
        profile["contact_hidden"] = False
        if has_contact_unlock and unlock_expires_at:
            profile["unlock_expires_at"] = unlock_expires_at
...
@router.post("/{user_id}/unlock-contact")
async def unlock_contact(user_id: str, current_user: dict = Depends(get_current_user)):
    """
    Unlock contact info for a user (7-day access).
    Skipping payment integration as per user request.
    """
    # Validate target user exists
    target_user = await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "role": 1})
    if not target_user:
        raise HTTPException(status_code=404, detail="User not found")
    
    # Prevent unlocking own contact
    if user_id == current_user["id"]:
        raise HTTPException(status_code=400, detail="Cannot unlock your own contact")
    ...
    existing_unlock = await db.contact_unlocks.find_one({
        "viewer_id": viewer_id,
        "target_user_id": user_id,
    ...
    unlock_doc = {
        ...
        "payment_amount": 0.0  # $0 - skipping Square payment
    }
    
    await db.contact_unlocks.insert_one(unlock_doc)
    ...
    return {
        "message": "Contact unlocked for 7 days",
        "expires_at": expires_at.isoformat(),
...
async def search_crew(
    ...
    # Skill filter (level 3) — searches both the skill field and free-form skills array
    if skill:
    ...
    # Expand category to all its trade names as a regex union
    if category and not trade:
        import re as _re
        cat_doc = await db.trade_categories.find_one(
            {"name": {"$regex": f"^{_re.escape(category)}$", "$options": "i"}}, {"_id": 0}
        )
        if cat_doc:
            trades_in_cat = await db.trades.find(
                {"category_id": cat_doc["id"]}, {"_id": 0, "name": 1}
            ).to_list(200)
            names = [t["name"] for t in trades_in_cat if t.get("name")]
            if names:
                trade = "|".join(_re.escape(n) for n in names)
    ...
    if available_only:
        # availability=False explicitly hides crew regardless of is_online (Issue 7)
