...
def _sum_txs(txs: list, start: datetime) -> float:
    """Sum completed transaction amounts from a start datetime."""
    total = 0.0
    for tx in txs:
        if tx.get("status") not in ("completed", "paid"):
            continue
        try:
            created = datetime.fromisoformat(tx["created_at"].replace("Z", ""))
            if created >= start:
...
def _sum_all(txs: list) -> float:
    """Sum all completed transactions regardless of date."""
    return round(sum(
        tx.get("amount", 0.0) for tx in txs
        if tx.get("status") in ("completed", "paid")
    ), 2)
...
async def square_pay(req: SquarePaymentRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Demo mode: when no Square token configured, simulate success
    if not SQUARE_ACCESS_TOKEN:
        tx_id = str(uuid.uuid4())
        await activate_subscription(current_user["id"], req.plan, tx_id, "square_demo")
        return {
    ...
    body = {
        ...
        "amount_money": {"amount": plan_info["amount"], "currency": "USD"},
    ...
    try:
        result = square_client.payments.create(body=body)
...
@router.get("/history")
async def payment_history(current_user: dict = Depends(get_current_user), all: bool = False):
    # Admin all-users view
    if all and current_user["role"] in ("admin", "superadmin"):
        ...
        for tx in txs:
            ...
            if uid not in user_cache:
                ...
                user_cache[uid] = u or {}
            tx["user_name"] = user_cache[uid].get("name", "—")
            tx["user_email"] = user_cache[uid].get("email", "—")
            # Normalize: boost transactions store payment_status, not status
            if "status" not in tx:
                tx["status"] = tx.get("payment_status", "completed")
        now = datetime.now(timezone.utc)
        ...
        return {
            "transactions": txs,
            "totals": {
                "daily":    _sum_txs(txs, day_start),
                "weekly":   _sum_txs(txs, week_start),
                "monthly":  _sum_txs(txs, month_start),
                "yearly":   _sum_txs(txs, year_start),
                "all_time": _sum_all(txs),
            },
        }
    ...
    # Normalize boost transactions (payment_status → status)
    for tx in txs:
        if "status" not in tx:
            tx["status"] = tx.get("payment_status", "completed")
