...
export default function ProfilePage() {
  ...
  const activateBoost = async () => {
    const price = boostStatus?.price ?? 4.99;
    if (!window.confirm(`Activate 7-day Profile Boost for $${price.toFixed(2)}? This is a one-time payment.`)) return;
    try {
  ...
  const missingFields = [
    { key: "photo",     label: "Photo",       check: !!(user?.profile_photo || user?.logo) },
    { key: "phone",     label: "Phone",       check: !!user?.phone },
    { key: "address",   label: "Address",     check: !!user?.address },
    { key: "bio",       label: "Bio",         check: !!user?.bio },
    { key: "skills",    label: "Trade/Skills",check: !!(user?.skills?.length || user?.trade) },
    { key: "portfolio", label: "Portfolio",   check: portfolio.length > 0 },
  ].filter(f => !f.check);
            ...
            <div className="card p-6">
                ...
                {editing ? (
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
              ...
              {editing ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: formatPhone(e.target.value) }))}
                        className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#0000FF] dark:bg-slate-800 dark:text-white"
                        placeholder="(555) 000-0000" data-testid="profile-phone-input" />
                    </div>
                  </div>
                  ...
                  {user?.role === "crew" && (
                    <div>
                      <label className="block text-sm font-semibold text-[#050A30] dark:text-white mb-1">Trade & Skill</label>
                      <DisciplineSelect
                        grouped={grouped}
                        value={{ discipline: form.discipline || "", trade: form.trade || "", skill: form.skill || "" }}
                        onChange={v => setForm(f => ({ ...f, discipline: v.discipline, trade: v.trade, skill: v.skill }))}
                        data-testid="profile-discipline-select"
                      />
                    </div>
                  )}
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <DisciplineSelect
                          ...
                          value={{ discipline: form.discipline || "", trade: form.trade || "", skill: form.skill || "" }}
                          onChange={v => setForm(f => ({ ...f, discipline: v.discipline, trade: v.trade, skill: v.skill }))}
                          data-testid="profile-contractor-discipline-select"
                        />
                      </div>
                      <div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={item.label} className="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-800">
                      ...
                      {item.href
                        ? <a href={item.href} className="text-sm font-semibold text-[#0000FF] hover:underline">{item.value}</a>
                        : <span className="text-sm font-semibold text-[#050A30] dark:text-white">{item.value}</span>
                      }
                    </div>
                  ))}
                  {user?.bio && (
