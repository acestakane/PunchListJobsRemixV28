...
export default function ArchivePage() {
  ...
  if (!user || !["contractor", "admin", "superadmin", "crew"].includes(user.role)) {
    return (
            ...
            <div className="flex gap-3 mt-6">
              ...
              {isCrew && (
                <button onClick={() => setPreview(null)}
                  className="flex-1 py-2.5 rounded-xl font-bold text-slate-600 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 text-sm">
                  Close
                </button>
              )}
            </div>
